        ...
        )
        {
            ...
            {//处理命令序列
                ...
                )
                {
                    ...
                    $udid = $db->get_var("select mid from mdevice where mid=\"$mid\"");
                    $db->query('INSERT INTO assi_info(`assid`,`order`,`cmdline`,`did`) VALUES ('.$scene_id.','.$no.',\''.json_encode($cmdline).'\',\''.strtoupper($cmdline["params"]["did"]).'\')');
                }
                else
                {
