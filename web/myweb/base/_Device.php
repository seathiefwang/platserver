...
EOD
			...
			//查找所有设备的数据库主表
			if (!$db->query(<<<EOD
select mid from alldevice where mid="$did"
EOD
			)) {
...
EOD
    ...
    public static function listByHouse($mid)
    {

        $db = Db::init();
        $re = array("devices"=>array());
        $results = $db->get_results(<<<EOD
select roomid from room where mid=$mid 
and exists(select mid from mdevice where mid=$mid) 
EOD
    );
        if (isset($results[0]))
        {
