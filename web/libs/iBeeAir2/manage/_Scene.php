...
class _Scene
{
    ...
    public static function _Create($params)
    {
        ...
        )
        {
            ...
            if($db->get_results('SELECT * FROM scene WHERE name="'.$name.'" and user="'.$user.'";'))
            { //情景模式已经存在
    ...
    public static function _Destroy($params)
    {
        ...
        )
        {
            ...
            if ($db->query(<<<EOD
delete from scene where user="$user" and sceneid="$sceneid"
EOD
        ))
            {//成功
                $db->query(<<<EOD
...
EOD
    ...
    public static function _List($params)
    {
        ...
        $results = $db->get_results('select * from scene where user="'.$user.'" order by sceneid ');
        $scenes = array();
        if (isset($results[0]))
        {
